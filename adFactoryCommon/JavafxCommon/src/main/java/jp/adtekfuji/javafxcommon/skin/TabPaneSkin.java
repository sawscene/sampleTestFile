...
package jp.adtekfuji.javafxcommon.skin;
...
import jp.adtekfuji.javafxcommon.util.ResourceUtil;
...
public class TabPaneSkin extends SkinBase<TabPane> {
     ...
     * nodes into the Control children list, as well as the necessary input
     * mappings for handling key, mouse, etc events.
     *
    ...
    public boolean isNavigationVisible() {
        return tabHeaderArea.navigationButtons.isVisible();
    }
    ...
    /**
     * Determines if the specified tab can be closed.
     * 
    ...
    private void closeTab(Tab tab) {
        TabPane tabPane = (TabPane) getNode();
    ...
    private void selectTab(Tab tab) {
        ((TabPane)getNode()).getSelectionModel().select(tab);
    }
    ...
    public double getNavigationButtonsWidth() {
        return snapSizeX(tabHeaderArea.navigationButtons.prefWidth(-1));
    }
    
    public double getControlButtonsWidth() {
        return snapSizeX(tabHeaderArea.controlButtons.prefWidth(-1));
    }
    ...
    /* ************************************************************************
     *
     * TabNavigationButtons: buttons to navigate between tabs
     *
     **************************************************************************/
    class TabNavigationButtons extends StackPane {
        private StackPane inner;
        private Label leftArrow;
        private Pane leftArrowBtn;
        private Label rightArrow;
        private Pane rightArrowBtn;

        public TabNavigationButtons() {

            getStyleClass().setAll("tab-navigation-buttons");

            leftArrowBtn = new Pane();
            leftArrowBtn.getStyleClass().setAll("tab-left-button");
            leftArrow = new Label("◀");
            leftArrow.getStyleClass().setAll("arrow");
            leftArrowBtn.getChildren().add(leftArrow);
            leftArrowBtn.setOnMouseClicked(me -> {
                TabHeaderArea headerArea = TabPaneSkin.this.tabHeaderArea;
                double currentOffset = headerArea.getScrollOffset();
                if (currentOffset >= 0) {
                    headerArea.setScrollOffset(calculateMaxScrollOffset(headerArea));
                } else {
                    headerArea.setScrollOffset(currentOffset + 80);
                }
            });

            rightArrowBtn = new Pane();
            rightArrowBtn.getStyleClass().setAll("tab-right-button");
            rightArrow = new Label("▶");
            rightArrow.getStyleClass().setAll("arrow");
            rightArrowBtn.getChildren().add(rightArrow);
            rightArrowBtn.setOnMouseClicked(me -> {
                TabHeaderArea headerArea = TabPaneSkin.this.tabHeaderArea;
                double currentOffset = headerArea.getScrollOffset();
                if (currentOffset <= calculateMaxScrollOffset(headerArea)) {
                    headerArea.setScrollOffset(0);
                } else {
                    headerArea.setScrollOffset(currentOffset - 80);
                }
            });
            ...
            inner = new StackPane() {
                @Override protected double computePrefWidth(double height) {
                    double pw = 0.0F;
                    pw += snapSize(leftArrow.prefWidth(getHeight())) + snapSize(leftArrowBtn.prefWidth(getHeight()));
                    pw += snapSize(rightArrow.prefWidth(getHeight())) + snapSize(rightArrowBtn.prefWidth(getHeight()));
                    if (pw > 0) {
                ...
                @Override protected double computePrefHeight(double width) {
                    double height = 0.0F;
                    height = Math.max(height, snapSize(leftArrowBtn.prefHeight(width)));
                    height = Math.max(height, snapSize(rightArrowBtn.prefHeight(width)));
                    if (height > 0) {
                ...
                @Override protected void layoutChildren() {
                    ...
                    double btnWidth = (snapSize(getWidth()) - x - snappedRightInset()) / 2;
                    double btnHeight = snapSize(getHeight()) - y - snappedBottomInset();

                    // Position left arrow button (left button)
                    leftArrowBtn.resize(btnWidth, btnHeight);
                    positionInArea(leftArrowBtn, x, y, btnWidth, btnHeight, /*baseline ignored*/0,
                            HPos.CENTER, VPos.CENTER);

                    // Position left arrow within button
                    double arrowWidth = snapSize(leftArrow.prefWidth(-1));
                    double arrowHeight = snapSize(leftArrow.prefHeight(-1));
                    leftArrow.resize(arrowWidth, arrowHeight);
                    positionInArea(leftArrow, leftArrowBtn.snappedLeftInset(), leftArrowBtn.snappedTopInset(),
                            btnWidth - leftArrowBtn.snappedLeftInset() - leftArrowBtn.snappedRightInset(),
                            btnHeight - leftArrowBtn.snappedTopInset() - leftArrowBtn.snappedBottomInset(),
                            /*baseline ignored*/0, HPos.CENTER, VPos.CENTER);

                    // Position right arrow button (right button)
                    rightArrowBtn.resize(btnWidth, btnHeight);
                    positionInArea(rightArrowBtn, x + btnWidth, y, btnWidth, btnHeight, /*baseline ignored*/0,
                            HPos.CENTER, VPos.CENTER);

                    // Position right arrow within button
                    arrowWidth = snapSize(rightArrow.prefWidth(-1));
                    arrowHeight = snapSize(rightArrow.prefHeight(-1));
                    rightArrow.resize(arrowWidth, arrowHeight);
                    positionInArea(rightArrow, rightArrowBtn.snappedLeftInset(), rightArrowBtn.snappedTopInset(),
                            btnWidth - rightArrowBtn.snappedLeftInset() - rightArrowBtn.snappedRightInset(),
                            btnHeight - rightArrowBtn.snappedTopInset() - rightArrowBtn.snappedBottomInset(),
                            /*baseline ignored*/0, HPos.CENTER, VPos.CENTER);
                }
            };
            inner.getStyleClass().add("container");
            inner.getChildren().addAll(leftArrowBtn, rightArrowBtn);

            getChildren().add(inner);
        }
        
        private double calculateMaxScrollOffset(TabHeaderArea headerArea) {
            double headersPrefWidth = snapSize(headerArea.headersRegion.prefWidth(-1));
            double controlTabWidth = snapSize(tabHeaderArea.controlButtons.prefWidth(-1));
            double navigationTabWidth = snapSize(tabHeaderArea.navigationButtons.prefWidth(-1));
            double visibleWidth = snapSize(headerArea.getWidth()) - controlTabWidth - navigationTabWidth - headerArea.firstTabIndent() - SPACER;
            return visibleWidth - headersPrefWidth;
        }
...
//            inner.requestLayout();
    ...
    class TabHeaderArea extends StackPane {
        ...
        private TabNavigationButtons navigationButtons;
        ...
        public TabHeaderArea() {
            ...
            navigationButtons = new TabNavigationButtons();
            navigationButtons.setVisible(true);

            getChildren().addAll(headerBackground, headersRegion, controlButtons, navigationButtons);
        ...
        private void updateHeaderClip() {
            ...
            double navigationButtonPrefWidth = snapSizeX(navigationButtons.prefWidth(-1));
            ...
            maxWidth = snapSizeX(getWidth()) - controlButtonPrefWidth - navigationButtonPrefWidth - SPACER - clipOffset;
            if (tabPosition.equals(Side.LEFT) || tabPosition.equals(Side.BOTTOM)) {
        ...
        private boolean tabsFit() {
            ...
            double navigationTabWidth = snapSizeX(navigationButtons.prefWidth(-1));
            double visibleWidth = headerPrefWidth + controlTabWidth + navigationTabWidth + firstTabIndent() + SPACER;
        ...
        private void ensureSelectedTabIsVisible() {
            ...
            double navigationTabWidth = snapSizeX(navigationButtons.prefWidth(-1));
            double visibleWidth = tabPaneWidth - controlTabWidth - navigationTabWidth - firstTabIndent() - SPACER;
        ...
        private void setScrollOffset(double newScrollOffset) {
            ...
            double tabPaneWidth = snapSizeX(isHorizontal() ? getSkinnable().getWidth() : getSkinnable().getHeight());
            double controlTabWidth = snapSizeX(controlButtons.getWidth());
            double navigationTabWidth = snapSizeX(navigationButtons.getWidth());
            double visibleWidth = tabPaneWidth - controlTabWidth - navigationTabWidth - firstTabIndent() - SPACER;
        ...
        @Override protected void layoutChildren() {
            ...
            double headersPrefWidth = snapSizeX(headersRegion.prefWidth(-1));
            double headersPrefHeight = snapSizeY(headersRegion.prefHeight(-1));
            boolean tabsFit = tabsFit();
            navigationButtons.setVisible(!tabsFit);
            ...
            double navigationBtnWidth = snapSizeX(navigationButtons.prefWidth(-1));
            double navigationBtnHeight = navigationButtons.prefHeight(navigationBtnWidth);
            navigationButtons.resize(navigationBtnWidth, navigationBtnHeight);
            ...
            double navigationStartX = 0;
            double navigationStartY = 0;
            ...
            if (tabPosition.equals(Side.TOP)) {
                ...
                navigationStartX = controlStartX - navigationBtnWidth;
                navigationBtnHeight = Math.max(navigationBtnHeight, headersPrefHeight);
                navigationStartY = snapSizeY(getHeight()) - navigationBtnHeight - bottomInset;
                ...
                navigationStartX = w - navigationBtnWidth + topInset;
                navigationStartY = snapSizeY(getHeight()) - navigationBtnHeight - leftInset;
                ...
                navigationStartX = rightInset;
                navigationBtnHeight = Math.max(navigationBtnHeight, headersPrefHeight);
                navigationStartY = snapSizeY(getHeight()) - navigationBtnHeight - topInset;
                ...
                navigationStartX = leftInset;
                navigationStartY = snapSizeY(getHeight()) - navigationBtnHeight - rightInset;
            }
            ...
            navigationButtons.resize(navigationBtnWidth, navigationBtnHeight);
            positionInArea(navigationButtons, navigationStartX, navigationStartY, navigationBtnWidth, navigationBtnHeight,
                    /*baseline ignored*/0, HPos.CENTER, VPos.CENTER);
    ...
    class TabHeaderSkin extends StackPane {
        ...
        // List to store all registered listeners for cleanup
        private final List<Runnable> registeredListeners = new ArrayList<>();


        // Helper method to register a change listener with a lambda that only needs the new value
        private <T> void registerChangeListener(javafx.beans.value.ObservableValue<T> property, java.util.function.Consumer<T> consumer) {
            javafx.beans.value.ChangeListener<T> listener = (obs, oldVal, newVal) -> consumer.accept(newVal);
            property.addListener(listener);
            registeredListeners.add(() -> property.removeListener(listener));
        }
        ...
        public TabHeaderSkin(final Tab tab) {
            ...
            closeBtn.setAccessibleText(ResourceUtil.getControlString("Accessibility.title.TabPane.CloseButton"));
            ...
            closeBtn.setOnMousePressed(new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent me) {
                    Tab tab = getTab();
                    if (me.getButton().equals(MouseButton.PRIMARY) && canCloseTab(tab)) {
                        closeTab(tab);
                        setOnMousePressed(null);
                        me.consume();
                    }
...
//                        }
                    ...
                    final int vPadding =  3;
                    final int hPadding =  1;
            ...
            registerChangeListener(tab.closableProperty(), e -> {
                inner.requestLayout();
                requestLayout();
            });
            registerChangeListener(tab.selectedProperty(), e -> {
            ...
            registerChangeListener(tab.textProperty(),e -> label.setText(getTab().getText()));
            registerChangeListener(tab.graphicProperty(), e -> label.setGraphic(getTab().getGraphic()));
            registerChangeListener(tab.tooltipProperty(), e -> {
            ...
            registerChangeListener(tab.disabledProperty(), e -> {
                updateTabDisabledState();
            });
            registerChangeListener(tab.getTabPane().disabledProperty(), e -> {
                updateTabDisabledState();
            });
            registerChangeListener(tab.styleProperty(), e -> setStyle(tab.getStyle()));

            tab.getStyleClass().addListener(weakStyleClassListener);

            registerChangeListener(getSkinnable().tabClosingPolicyProperty(),e -> {
                inner.requestLayout();
                requestLayout();
            });
            registerChangeListener(getSkinnable().sideProperty(),e -> {
            ...
            registerChangeListener(getSkinnable().rotateGraphicProperty(), e -> updateGraphicRotation());
            registerChangeListener(getSkinnable().tabMinWidthProperty(), e -> {
                requestLayout();
                getSkinnable().requestLayout();
            });
            registerChangeListener(getSkinnable().tabMaxWidthProperty(), e -> {
                requestLayout();
                getSkinnable().requestLayout();
            });
            registerChangeListener(getSkinnable().tabMinHeightProperty(), e -> {
                requestLayout();
                getSkinnable().requestLayout();
            });
            registerChangeListener(getSkinnable().tabMaxHeightProperty(), e -> {
                requestLayout();
                getSkinnable().requestLayout();
            });
        ...
        private void dispose() {
            ...
            for (Runnable cleanup : registeredListeners) {
                cleanup.run();
            }
